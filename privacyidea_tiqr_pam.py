...
def generate_qr_small(modules, header):
    ...
    qr_str = before_line
    qr_str += qr_half_char(False, False)
    for x in range(0, len(modules[0])):
        qr_str += qr_half_char(False, False)
    qr_str += qr_half_char(False, False) + after_line + '\n'

    for y in range(0, len(modules)//2+1):
        qr_str += before_line + qr_half_char(False, False)
        for x in range(0, len(modules[0])):
            qr_str += qr_half_char(
                modules[y*2][x],
...
def generate_qr_big(modules, header):
    #before_line = config['qr']['before_line']
    #after_line = config['qr']['after_line']
    ...
    qr_str += qr_full_char(False)
    for x in range(0, len(modules[0])):
        qr_str += qr_full_char(False)
    qr_str += qr_full_char(False) + after_line + '\n'

    for y in range(0, len(modules)):
        qr_str += before_line + qr_full_char(False)
        for x in range(0, len(modules[0])):
            qr_str += qr_full_char(modules[y][x])
        qr_str += qr_full_char(False) + after_line + '\n'
